...
exports.getProperties = async (req, res, next) => {
  try {
    ...
    // Chỉ hiển thị tin đã được duyệt (trừ khi là admin)
...
/**
 * @desc    Lấy danh sách tin đăng chờ duyệt (Admin only)
...
/**
 * @desc    Duyệt tin đăng (Admin only)
...
exports.approveProperty = async (req, res, next) => {
  try {
    ...
    if (property.approvalStatus === 'approved') {
      return res.status(400).json({
        success: false,
        error: 'Tin đăng này đã được duyệt'
      });
    }
    ...
    // TODO: Gửi thông báo cho landlord
...
/**
 * @desc    Từ chối tin đăng (Admin only)
