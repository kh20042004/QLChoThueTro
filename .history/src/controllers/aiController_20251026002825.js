...
exports.searchWithAI = async (req, res, next) => {
  try {
    ...
    // Nếu có landmark (địa danh), tìm trong title hoặc description
    if (filters.landmark) {
      // Tìm trong title, description hoặc address
      const landmarkRegex = new RegExp(filters.landmark, 'i');
      const landmarkCondition = {
        $or: [
          { title: landmarkRegex },
          { description: landmarkRegex },
          { 'address.street': landmarkRegex },
          { 'address.ward': landmarkRegex }
        ]
      };
      
      // Kết hợp với location query nếu có
      if (searchQuery.$or) {
        searchQuery.$and = [
          { $or: searchQuery.$or },
          landmarkCondition
        ...
        searchQuery.$or = landmarkCondition.$or;
      }
    }
