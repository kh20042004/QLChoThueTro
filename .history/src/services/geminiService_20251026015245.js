...
{
  "suggestedPrice": <số tiền đề xuất>,
  "priceRange": {
    "min": <giá tối thiểu>,
    "max": <giá tối đa>
  },
...
- Diện tích lớn hơn thường giá cao hơn theo tỷ lệ`;
    ...
    // Validate và fix giá nếu bị 0 hoặc null
    if (!prediction.suggestedPrice || prediction.suggestedPrice === 0) {
      console.warn('⚠️ Giá đề xuất bị 0 hoặc null, tính toán lại...');
      ...
      console.log('✅ Giá sau khi fix:', prediction.suggestedPrice);
    }
