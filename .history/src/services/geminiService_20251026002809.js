...
/**
 * Parse natural language search query to filters
 * @param {String} query - Câu tìm kiếm tự nhiên
 * @returns {Object} - Filters và summary
 */
exports.parseSearchQuery = async (query) => {
  try {
    const prompt = `Bạn là AI phân tích yêu cầu tìm kiếm phòng trọ tại Việt Nam. Phân tích câu sau và trả về JSON:

Câu tìm kiếm: "${query}"

QUAN TRỌNG - Phân tích địa điểm:
- Nếu có nhắc đến trường đại học, bệnh viện, chợ, ga tàu, sân bay, khu công nghiệp, hoặc địa danh cụ thể → Tìm quận/huyện tương ứng
- Ví dụ: "gần ĐH Bách Khoa" → Quận Thủ Đức
- Ví dụ: "gần Chợ Bến Thành" → Quận 1
- Ví dụ: "gần Vincom" → tùy theo Vincom nào (VD: Vincom Đồng Khởi → Quận 1)
- Ví dụ: "gần Tân Sơn Nhất" → Tân Bình
- Ví dụ: "gần bệnh viện Chợ Rẫy" → Quận 5
- Nếu không xác định được → để trống

Danh sách địa danh phổ biến ở TP.HCM:
- Trường ĐH: Bách Khoa (Thủ Đức), Kinh Tế (Thủ Đức), KHTN (Thủ Đức), Sư Phạm (Quận 5), Y Dược (Quận 5)
- Chợ: Bến Thành (Q1), An Đông (Q5), Tân Định (Q1), Bình Tây (Q6)
- Bệnh viện: Chợ Rẫy (Q5), 115 (Q10), Từ Dũ (Q1), Nhi Đồng 1 (Q10)
- Sân bay: Tân Sơn Nhất (Tân Bình)
- KCN: Tân Thuận (Q7), Vĩnh Lộc (Bình Chánh)

Trả về JSON theo format SAU (CHÍNH XÁC):
{
  "propertyType": "phong-tro|nha-nguyen-can|can-ho|chung-cu-mini|homestay hoặc rỗng",
  "location": "tên quận/huyện hoặc rỗng (VD: Quận 1, Thủ Đức, Gò Vấp, Tân Bình)",
  "landmark": "địa danh được nhắc đến hoặc rỗng (VD: Đại học Bách Khoa, Chợ Bến Thành)",
  "priceMin": số hoặc null,
  "priceMax": số hoặc null,
  "area": số hoặc null,
  "bedrooms": số hoặc null,
  "amenities": ["wifi", "airConditioner", "parking", "kitchen", "waterHeater", "washingMachine", "fridge", "security"],
  "summary": "Tóm tắt ngắn gọn yêu cầu của người dùng, bao gồm cả địa danh nếu có"
}

Lưu ý:
- propertyType: phong-tro, nha-nguyen-can, can-ho, chung-cu-mini, homestay
- location: CHỈ tên quận/huyện (VD: "Quận 1", "Gò Vấp", "Thủ Đức", "Tân Bình")
- landmark: Tên địa danh cụ thể nếu được nhắc (VD: "Đại học Bách Khoa", "Chợ Bến Thành")
- priceMin, priceMax: đơn vị triệu VND (VD: 2 = 2 triệu)
- amenities: chỉ các tiện ích được nhắc đến
- summary: Mô tả đầy đủ, VD: "Tìm phòng trọ giá rẻ gần Đại học Bách Khoa, Thủ Đức"
- Chỉ trả về JSON, KHÔNG kèm markdown, KHÔNG kèm giải thích`;
    ...
    return {
      success: true,
      filters: filters,
      summary: filters.summary || query
    };
  } catch (error) {
    console.error('Gemini AI Parse Error:', error);
    return {
      success: false,
      error: 'Không thể phân tích yêu cầu. Vui lòng thử lại với mô tả rõ ràng hơn.',
      filters: null
    };
  }
};
