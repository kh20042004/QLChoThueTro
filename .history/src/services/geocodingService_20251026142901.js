...
async function getCoordinatesFromAddress(street, ward, district, city) {
    try {
        // Thử nhiều format địa chỉ khác nhau
        const addressFormats = [
            `${street}, ${district}, ${city}, Vietnam`,  // Format 1: Bỏ phường
            `${street}, ${ward}, ${district}, ${city}`,  // Format 2: Đầy đủ không có Vietnam
            `${street}, ${district}, ${city}`,           // Format 3: Ngắn gọn
            `${district}, ${city}, Vietnam`              // Format 4: Chỉ quận/huyện
        ];

        for (let i = 0; i < addressFormats.length; i++) {
            const address = addressFormats[i];
            console.log(`🔍 [Thử ${i + 1}/${addressFormats.length}] Tìm tọa độ: ${address}`);
            ...
            try {
                ...
                if (response.data && response.data.length > 0) {
                    ...
                    console.log(`✅ Tìm thấy tọa độ: [${coordinates.lng}, ${coordinates.lat}] (accuracy: ${coordinates.accuracy.toFixed(2)})`);
                    return coordinates;
                }

                // Delay 1 giây giữa các lần thử để tránh rate limit
                if (i < addressFormats.length - 1) {
                    await delay(1000);
                }

            } catch (err) {
                console.warn(`   ⚠️ Lỗi với format ${i + 1}: ${err.message}`);
                continue;
            }
        }

        // Nếu tất cả format đều thất bại, dùng mapping mặc định
        console.log('⚠️ Không tìm thấy với mọi format, sử dụng mapping mặc định...');
