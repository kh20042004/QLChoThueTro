            ...
            <div class="p-5">
                ...
                <div class="flex justify-between items-center">
                    ...
                    <div class="flex gap-2">
                        <button onclick="showPropertyLocation('${property._id}', '${property.title.replace(/'/g, "\\'")}', ${property.location?.coordinates?.[1] || 0}, ${property.location?.coordinates?.[0] || 0})" 
...
/**
 * Hiển thị vị trí property trên bản đồ
 * @param {String} propertyId - ID của property
 * @param {String} propertyTitle - Tiêu đề property
 * @param {Number} lat - Latitude
 * @param {Number} lng - Longitude
 */
function showPropertyLocation(propertyId, propertyTitle, lat, lng) {
    // Kiểm tra tọa độ hợp lệ
    if (!lat || !lng || lat === 0 || lng === 0) {
        alert('Phòng này chưa có thông tin vị trí chính xác.');
        return;
    }

    // Tạo modal nếu chưa tồn tại
    let modal = document.getElementById('propertyMapModal');
    if (!modal) {
        modal = document.createElement('div');
        modal.id = 'propertyMapModal';
        modal.className = 'fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center hidden';
        modal.innerHTML = `
            <div class="bg-white rounded-xl shadow-2xl w-full max-w-4xl mx-4 overflow-hidden">
                <div class="flex items-center justify-between p-4 bg-gray-800 text-white">
                    <h3 class="text-lg font-semibold" id="mapModalTitle">Vị trí phòng</h3>
                    <button onclick="closePropertyMap()" class="text-white hover:text-gray-300 text-2xl">
                        <i class="fas fa-times"></i>
                    </button>
                </div>
                <div id="propertyMap" class="w-full h-96"></div>
                <div class="p-4 bg-gray-50 flex gap-2">
                    <a id="mapDirectionsLink" target="_blank" class="flex-1 px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors text-center">
                        <i class="fas fa-directions mr-2"></i>Chỉ đường
                    </a>
                    <button onclick="closePropertyMap()" class="px-4 py-2 bg-gray-300 text-gray-800 rounded-lg hover:bg-gray-400 transition-colors">
                        Đóng
                    </button>
                </div>
            </div>
    ...
    // Cập nhật tiêu đề
    document.getElementById('mapModalTitle').textContent = propertyTitle;
    ...
    setTimeout(() => {
        if (!propertyMap) {
            ...
            propertyMap = L.map('propertyMap').setView([lat, lng], 15);
            ...
            propertyMarker = L.marker([lat, lng]).addTo(propertyMap)
                .bindPopup(`<b>${propertyTitle}</b><br><a href="/properties/${propertyId}" class="text-blue-500 hover:underline">Xem chi tiết</a>`)
                .openPopup();
        } else {
        ...
        // Cập nhật link chỉ đường (Google Maps)
...
function closePropertyMap() {
    const modal = document.getElementById('propertyMapModal');
    if (modal) {
