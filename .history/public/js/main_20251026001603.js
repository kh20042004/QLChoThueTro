...
function displayAISearchResult(filters, summary, properties, count) {
    ...
    // Build analysis HTML
    let html = `<p class="mb-2"><strong>📝 ${summary}</strong></p><ul class="space-y-1 ml-4">`;
    ...
    if (filters.propertyType) {
        ...
        html += `<li>🏠 <strong>Loại:</strong> ${typeMap[filters.propertyType] || filters.propertyType}</li>`;
    }
    
    if (filters.location) {
        html += `<li>📍 <strong>Vị trí:</strong> ${filters.location}</li>`;
    }
    ...
    if (filters.priceMin || filters.priceMax) {
        ...
        html += `<li>💰 <strong>Giá:</strong> ${priceText}</li>`;
    }
    
    if (filters.area) {
        html += `<li>📐 <strong>Diện tích:</strong> Khoảng ${filters.area}m²</li>`;
    }
    
    if (filters.bedrooms) {
        html += `<li>🛏️ <strong>Phòng ngủ:</strong> ${filters.bedrooms} phòng</li>`;
    }
    ...
    if (filters.amenities && filters.amenities.length > 0) {
        ...
        html += `<li>✨ <strong>Tiện ích:</strong> ${amenityList}</li>`;
    }
    ...
    // Store filters for apply action
