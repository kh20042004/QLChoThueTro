...
class AISearch {
    ...
    /**
     * Hiển thị kết quả phòng được gợi ý
     */
    showPropertyResults(properties) {
        const messagesContainer = this.modal.querySelector('#aiSearchMessages');
        if (!messagesContainer) return;
        ...
        properties.forEach(property => {
            ...
            const typeLabel = typeMap[property.propertyType] || 'Phòng trọ';

            const district = property.address?.district || '';
            const city = property.address?.city || '';

            cardsHTML += `
                <div class="ai-search-property-card" onclick="window.location.href='/properties/${property._id}'">
                    <div style="display: flex; gap: 12px;">
                        ...
                        <div style="flex: 1; min-width: 0;">
                            ...
                            <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 4px;">
                                <span style="font-size: 12px; padding: 2px 8px; background: #e5e7eb; border-radius: 4px; color: #4b5563;">
                                    ${typeLabel}
                                </span>
                                <span style="font-size: 13px; color: #6b7280;">
                                    <i class="fas fa-map-marker-alt" style="font-size: 11px;"></i> ${district}${city ? ', ' + city : ''}
                                </span>
                            </div>
                            <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 8px; font-size: 12px; color: #6b7280;">
                                <span><i class="fas fa-expand-arrows-alt"></i> ${property.area || 0}m²</span>
                                ${property.bedrooms ? `<span><i class="fas fa-bed"></i> ${property.bedrooms} PN</span>` : ''}
                                ${property.bathrooms ? `<span><i class="fas fa-bath"></i> ${property.bathrooms} WC</span>` : ''}
                            </div>
        ...
        resultsDiv.innerHTML = `
            <div class="ai-search-message ai">
                <div class="ai-search-avatar">🏠</div>
                <div class="ai-search-message-content" style="max-width: 85%;">
                    ...
                    <div style="margin-top: 12px; padding-top: 12px; border-top: 1px solid #e5e7eb; font-size: 13px; color: #6b7280;">
                        💡 Click vào phòng để xem chi tiết hoặc tiếp tục hỏi tôi để tìm phòng khác nhé!
                    </div>
                </div>
            </div>
        `;
