...
// Global functions for AI Search Modal
window.openAISearchModal = function() {
    ...
    if (modal) {
        ...
        document.getElementById('aiSearchError').classList.add('hidden');
        document.getElementById('aiSearchBtn').classList.remove('hidden');
...
window.setAISearchExample = function(text) {
...
window.performAISearch = async function() {
    ...
    if (!query) {
        showAISearchError('Vui lòng nhập yêu cầu tìm kiếm');
        return;
    }
    ...
    document.getElementById('aiSearchResult').classList.add('hidden');
    document.getElementById('aiSearchError').classList.add('hidden');
    document.getElementById('aiSearchLoading').classList.remove('hidden');
    ...
    try {
        ...
        if (data.success && data.data.filters) {
            displayAISearchResult(data.data.filters, data.data.summary, data.data.properties, data.data.count);
        } else {
            showAISearchError(data.error || 'Không thể phân tích yêu cầu');
        }
    } catch (error) {
        console.error('AI Search error:', error);
        showAISearchError('Có lỗi xảy ra. Vui lòng thử lại.');
    } finally {
        document.getElementById('aiSearchLoading').classList.add('hidden');
        document.getElementById('aiSearchBtn').disabled = false;
    }
};

function displayAISearchResult(filters, summary, properties, count) {
    const resultDiv = document.getElementById('aiSearchResult');
    const analysisDiv = document.getElementById('aiAnalysis');
    
    // Build analysis HTML
    let html = `<p class="mb-3"><strong>📝 ${summary}</strong></p>`;
    
    // Hiển thị số lượng phòng tìm thấy
    if (count > 0) {
        html += `<div class="bg-green-100 border border-green-300 rounded-lg p-3 mb-4">
            <p class="text-green-800 font-semibold">
                <i class="fas fa-check-circle mr-2"></i>Tìm thấy ${count} phòng phù hợp!
            </p>
        </div>`;
        
        // Hiển thị danh sách property cards (giống trang chủ)
        html += `<div class="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-[500px] overflow-y-auto pr-2">`;
        properties.forEach(prop => {
            const price = (prop.price / 1000000).toFixed(1);
            const image = prop.images && prop.images.length > 0 ? prop.images[0] : '/images/property-placeholder.jpg';
            const district = prop.address?.district || '';
            const city = prop.address?.city || '';
            
            // Tạo danh sách tiện nghi
            const amenities = [];
            if (prop.amenities) {
                if (prop.amenities.wifi) amenities.push('<span class="px-2 py-1 bg-gray-100 text-gray-700 text-xs rounded"><i class="fas fa-wifi mr-1"></i>Wifi</span>');
                if (prop.amenities.airConditioner) amenities.push('<span class="px-2 py-1 bg-gray-100 text-gray-700 text-xs rounded"><i class="fas fa-snowflake mr-1"></i>Điều hòa</span>');
                if (prop.amenities.parking) amenities.push('<span class="px-2 py-1 bg-gray-100 text-gray-700 text-xs rounded"><i class="fas fa-car mr-1"></i>Xe</span>');
                if (prop.amenities.kitchen) amenities.push('<span class="px-2 py-1 bg-gray-100 text-gray-700 text-xs rounded"><i class="fas fa-utensils mr-1"></i>Bếp</span>');
            }
            
            // Status badge
            const statusBadge = prop.status === 'available' 
            ...
            const typeLabel = typeMap[prop.propertyType] || 'Phòng trọ';
            
            html += `
                <div class="bg-white/90 backdrop-blur-sm rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1 cursor-pointer" onclick="window.open('/properties/${prop._id}', '_blank')">
                    <div class="relative">
                        <img src="${image}" alt="${prop.title}" class="w-full h-48 object-cover" onerror="this.src='/images/property-placeholder.jpg'">
                    ...
                    <div class="p-4">
                        <div class="flex justify-between items-start mb-2">
                            <h3 class="text-base font-semibold text-gray-900 truncate flex-1">${prop.title}</h3>
                            <div class="text-right ml-2">
                                <div class="text-lg font-bold text-gray-800">${price} triệu</div>
                                <div class="text-xs text-gray-500">/tháng</div>
                            </div>
                        </div>
                        
                        <p class="text-sm text-gray-500 mb-2 truncate">
                            <i class="fas fa-map-marker-alt mr-1"></i>${district}${city ? ', ' + city : ''}
                        </p>
                        
                        <div class="flex gap-3 mb-3 text-xs text-gray-600">
                            <span><i class="fas fa-expand-arrows-alt mr-1"></i>${prop.area || 0}m²</span>
                            <span><i class="fas fa-bed mr-1"></i>${prop.bedrooms || 0} PN</span>
                            <span><i class="fas fa-bath mr-1"></i>${prop.bathrooms || 0} WC</span>
                        </div>
                        ...
                        <button class="w-full px-4 py-2 bg-gray-800 text-white text-sm rounded-lg hover:bg-gray-700 transition-colors duration-300">
                            Xem chi tiết
                        </button>
                    </div>
                </div>
        ...
        // Thêm summary filters
        html += `<div class="mt-4 pt-3 border-t border-gray-200">
            <p class="text-sm text-gray-600 mb-2"><strong>Bộ lọc đã áp dụng:</strong></p>
        ...
        if (filters.propertyType) {
            ...
            html += `<li>🏠 ${typeMap[filters.propertyType] || filters.propertyType}</li>`;
        }
        
        if (filters.location) {
            html += `<li>📍 ${filters.location}</li>`;
        }
        
        if (filters.landmark) {
            html += `<li>🎯 Gần ${filters.landmark}</li>`;
        }
        
        if (filters.priceMin || filters.priceMax) {
            const priceText = filters.priceMin && filters.priceMax 
                ? `${filters.priceMin} - ${filters.priceMax} triệu`
                : filters.priceMin 
                ? `Từ ${filters.priceMin} triệu`
                : `Dưới ${filters.priceMax} triệu`;
            html += `<li>💰 ${priceText}</li>`;
        }
        
        if (filters.area) {
            html += `<li>📐 ~${filters.area}m²</li>`;
        }
        
        if (filters.bedrooms) {
            html += `<li>🛏️ ${filters.bedrooms} phòng ngủ</li>`;
        }
        ...
        if (filters.amenities && filters.amenities.length > 0) {
            ...
            const amenityList = filters.amenities.map(a => amenityMap[a] || a).join(', ');
            html += `<li>✨ ${amenityList}</li>`;
        }
        ...
        html += `<div class="bg-yellow-100 border border-yellow-300 rounded-lg p-4 text-center">
            <i class="fas fa-search text-yellow-600 text-3xl mb-2"></i>
            <p class="text-yellow-800 font-semibold mb-2">Không tìm thấy phòng phù hợp</p>
            <p class="text-yellow-700 text-sm">Thử điều chỉnh yêu cầu của bạn hoặc tìm kiếm với điều kiện khác</p>
        </div>`;
        
        // Vẫn hiển thị filters đã phân tích
        html += `<div class="mt-4 pt-3 border-t border-gray-200">
            <p class="text-sm text-gray-600 mb-2"><strong>AI đã hiểu yêu cầu:</strong></p>
            <ul class="space-y-1 text-sm text-gray-700 ml-4">`;
    ...
    window.currentAIFilters = filters;
    window.currentAIProperties = properties;
    
    // Show/hide buttons based on results
    document.getElementById('aiSearchBtn').classList.add('hidden');
    if (count > 0) {
...
function showAISearchError(message) {
    document.getElementById('aiSearchError').classList.remove('hidden');
    document.getElementById('aiSearchErrorMsg').textContent = message;
}

window.applyAIFilters = function() {
    if (!window.currentAIFilters) return;
    
    const filters = window.currentAIFilters;
    ...
    if (filters.priceMin) params.append('priceMin', filters.priceMin * 1000000);
    if (filters.priceMax) params.append('priceMax', filters.priceMax * 1000000);
