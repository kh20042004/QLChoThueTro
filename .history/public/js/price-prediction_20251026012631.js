...
class PricePrediction {
    ...
    validateFormData(data) {
        ...
        if (!data.location.city || data.location.city === '') {
            ...
            // Tìm step chứa field province
            const provinceField = document.getElementById('province');
            if (provinceField) {
                // Tìm step parent
                const step = provinceField.closest('.form-step');
                if (step && step.classList.contains('hidden')) {
                    // Chuyển sang step 2 (địa chỉ)
                    this.showToast('⚠️ Vui lòng điền thông tin Địa chỉ (Step 2) trước', 'warning');
                    ...
                    this.showToast('⚠️ Vui lòng chọn Tỉnh/Thành phố trước khi dự đoán giá', 'warning');
                    provinceField.scrollIntoView({ behavior: 'smooth', block: 'center' });
                    provinceField.focus();
                    setTimeout(() => {
                        provinceField.style.border = '2px solid #ef4444';
                ...
                this.showToast('⚠️ Vui lòng chọn Tỉnh/Thành phố', 'warning');
            }
            return false;
        }
