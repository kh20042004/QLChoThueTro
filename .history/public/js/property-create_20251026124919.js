...
function handleImageSelection(files) {
    ...
    Array.from(files).forEach(file => {
        ...
        reader.onload = (e) => {
            ...
            // Gọi AI phân tích ảnh khi có đủ 2 ảnh trở lên
            if (uploadedImages.length >= 2 && window.analyzeUploadedImages) {
                setTimeout(() => {
                    const imagePaths = uploadedImages.map(img => img.preview);
                    window.analyzeUploadedImages(imagePaths);
                }, 1500);
            }
        };
