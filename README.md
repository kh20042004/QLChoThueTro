# 🏠 Hệ Thống Cho Thuê Nhà/Phòng Trọ với AI

Một hệ thống quản lý cho thuê nhà/phòng trọ hiện đại, được xây dựng bằng Node.js, Express, MongoDB và tích hợp công nghệ AI.

## 📋 Mục Lục

- [Tính Năng](#tính-năng)
- [Công Nghệ](#công-nghệ)
- [Cài Đặt](#cài-đặt)
- [Cấu Hình](#cấu-hình)
- [Chạy Ứng Dụng](#chạy-ứng-dụng)
- [API Documentation](#api-documentation)
- [Cấu Trúc Thư Mục](#cấu-trúc-thư-mục)

## ✨ Tính Năng

### Người Dùng
- ✅ Đăng ký, đăng nhập, quên mật khẩu
- ✅ Tìm kiếm phòng trọ theo nhiều tiêu chí
- ✅ Xem chi tiết phòng với hình ảnh, địa chỉ, giá cả
- ✅ Đánh giá và nhận xét phòng đã thuê
- ✅ Đặt phòng và quản lý booking
- 🤖 Tìm kiếm thông minh bằng AI
- 💬 Chatbot hỗ trợ 24/7

### Chủ Nhà
- ✅ Đăng tin cho thuê phòng/nhà
- ✅ Quản lý danh sách phòng
- ✅ Theo dõi booking và thanh toán
- ✅ Xem thống kê và báo cáo
- 📸 Xác thực ảnh tự động bằng AI

### Admin
- ✅ Quản lý người dùng
- ✅ Quản lý tất cả phòng và booking
- ✅ Xem thống kê toàn hệ thống

### Tính Năng AI
- 🤖 Gợi ý phòng phù hợp dựa trên sở thích
- 📸 Nhận diện và phân loại ảnh phòng
- 💬 Chatbot tư vấn thông minh
- 🔍 Tìm kiếm ngữ nghĩa (Semantic Search)

## 🛠 Công Nghệ

### Backend
- **Node.js** - Runtime environment
- **Express.js** - Web framework
- **MySQL** - Relational database
- **Sequelize** - ORM cho MySQL
- **JWT** - Authentication
- **Bcrypt** - Mã hóa mật khẩu

### Frontend
- **HTML5** - Markup
- **CSS3** - Styling
- **Bootstrap 5** - CSS framework
- **JavaScript (ES6+)** - Programming
- **Font Awesome** - Icons

### AI & Machine Learning
- **TensorFlow.js** - Image recognition
- **OpenAI API** - Chatbot & NLP
- **Sharp** - Image processing

### Tools & Libraries
- **Multer** - File upload
- **Nodemailer** - Email service
- **Morgan** - Logging
- **Helmet** - Security
- **CORS** - Cross-origin resource sharing
- **Compression** - Gzip compression

## 📦 Cài Đặt

### Yêu Cầu Hệ Thống
- Node.js >= 16.x
- MySQL >= 8.0
- NPM hoặc Yarn

### Các Bước Cài Đặt

1. **Clone repository**
```bash
git clone https://github.com/yourusername/room-rental-system.git
cd room-rental-system
```

2. **Cài đặt dependencies**
```bash
npm install
```

3. **Tạo file .env**
```bash
cp .env.example .env
```

4. **Cấu hình file .env** (xem phần [Cấu Hình](#cấu-hình))

5. **Tạo database MySQL**
```sql
CREATE DATABASE room_rental_db CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
```

6. **Chạy MySQL Server**
```bash
# Đảm bảo MySQL đang chạy
# Windows: Services -> MySQL
# Linux: sudo service mysql start
# Mac: brew services start mysql
```

## ⚙️ Cấu Hình

Tạo file `.env` trong thư mục gốc với nội dung:

```env
# Server Configuration
PORT=3000
NODE_ENV=development

# MySQL Database Configuration
DB_HOST=localhost
DB_PORT=3306
DB_NAME=room_rental_db
DB_USER=root
DB_PASSWORD=your_mysql_password

# JWT Configuration
JWT_SECRET=your-super-secret-jwt-key-change-this-in-production
JWT_EXPIRE=7d
JWT_COOKIE_EXPIRE=7

# Email Configuration
EMAIL_HOST=smtp.gmail.com
EMAIL_PORT=587
EMAIL_USERNAME=your-email@gmail.com
EMAIL_PASSWORD=your-app-password
EMAIL_FROM=noreply@roomrental.com

# AI Configuration
OPENAI_API_KEY=your-openai-api-key-here

# File Upload Configuration
MAX_FILE_SIZE=5242880
ALLOWED_FILE_TYPES=image/jpeg,image/png,image/jpg

# Session Configuration
SESSION_SECRET=your-session-secret-key-change-this

# Client URL
CLIENT_URL=http://localhost:3000
```

### Cấu Hình MySQL
1. Cài đặt MySQL Server (nếu chưa có): https://dev.mysql.com/downloads/
2. Tạo database:
```sql
CREATE DATABASE room_rental_db CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
```
3. Cập nhật thông tin trong file `.env`:
   - `DB_USER`: username MySQL của bạn (mặc định: root)
   - `DB_PASSWORD`: password MySQL của bạn
   - `DB_NAME`: tên database (mặc định: room_rental_db)

### Lấy OpenAI API Key
1. Truy cập https://platform.openai.com
2. Đăng ký/Đăng nhập
3. Tạo API key tại https://platform.openai.com/api-keys

## 🚀 Chạy Ứng Dụng

### Development Mode
```bash
npm run dev
```

### Production Mode
```bash
npm start
```

Server sẽ chạy tại: `http://localhost:3000`

## 📚 API Documentation

### Authentication Endpoints

#### Đăng ký
```http
POST /api/auth/register
Content-Type: application/json

{
  "name": "Nguyễn Văn A",
  "email": "user@example.com",
  "phone": "0123456789",
  "password": "123456",
  "role": "user"
}
```

#### Đăng nhập
```http
POST /api/auth/login
Content-Type: application/json

{
  "email": "user@example.com",
  "password": "123456"
}
```

#### Lấy thông tin user
```http
GET /api/auth/me
Authorization: Bearer {token}
```

### Property Endpoints

#### Lấy danh sách phòng
```http
GET /api/properties?page=1&limit=10&propertyType=phong-tro&city=ha-noi
```

#### Lấy chi tiết 1 phòng
```http
GET /api/properties/:id
```

#### Tạo phòng mới (Landlord only)
```http
POST /api/properties
Authorization: Bearer {token}
Content-Type: application/json

{
  "title": "Phòng trọ cao cấp Q1",
  "description": "Phòng đẹp, đầy đủ tiện nghi",
  "propertyType": "phong-tro",
  "price": 3500000,
  "area": 25,
  "address": {
    "street": "123 Nguyễn Huệ",
    "ward": "Phường Bến Nghé",
    "district": "Quận 1",
    "city": "TP. Hồ Chí Minh"
  }
}
```

### Booking Endpoints

#### Tạo booking mới
```http
POST /api/bookings
Authorization: Bearer {token}
Content-Type: application/json

{
  "property": "property_id",
  "startDate": "2024-01-01",
  "endDate": "2024-12-31",
  "notes": "Ghi chú"
}
```

### Review Endpoints

#### Tạo đánh giá
```http
POST /api/reviews
Authorization: Bearer {token}
Content-Type: application/json

{
  "property": "property_id",
  "rating": 5,
  "title": "Phòng rất tốt",
  "comment": "Phòng sạch sẽ, chủ nhà thân thiện"
}
```

### AI Endpoints

#### Chatbot
```http
POST /api/ai/chatbot
Content-Type: application/json

{
  "message": "Tôi cần tìm phòng trọ gần trường đại học"
}
```

#### Tìm kiếm thông minh
```http
POST /api/ai/smart-search
Content-Type: application/json

{
  "query": "phòng trọ có điều hòa giá dưới 3 triệu"
}
```

## 📁 Cấu Trúc Thư Mục

```
room-rental-system/
├── .github/
│   └── copilot-instructions.md
├── public/
│   ├── css/
│   │   ├── main.css
│   │   └── auth.css
│   ├── js/
│   │   ├── main.js
│   │   ├── about.js
│   │   └── contact.js
│   ├── images/
│   └── uploads/
├── src/
│   ├── config/
│   │   ├── database.js
│   │   └── colors.js
│   ├── controllers/
│   │   ├── authController.js
│   │   └── propertyController.js
│   ├── middleware/
│   │   ├── auth.js
│   │   ├── errorHandler.js
│   │   └── upload.js
│   ├── models/
│   │   ├── User.js
│   │   ├── Property.js
│   │   ├── Booking.js
│   │   └── Review.js
│   ├── routes/
│   │   ├── authRoutes.js
│   │   ├── propertyRoutes.js
│   │   ├── bookingRoutes.js
│   │   ├── reviewRoutes.js
│   │   └── aiRoutes.js
│   ├── services/
│   │   └── (AI services sẽ được thêm vào)
│   ├── utils/
│   └── app.js
├── views/
│   ├── index.html
│   ├── about.html
│   ├── contact.html
│   ├── properties.html
│   ├── auth/
│   │   ├── login.html
│   │   └── register.html
│   └── partials/
├── .env.example
├── .gitignore
├── nodemon.json
├── package.json
├── server.js
└── README.md
```

## 🔐 Bảo Mật

- ✅ Mã hóa mật khẩu với Bcrypt
- ✅ JWT cho authentication
- ✅ Helmet.js cho HTTP headers security
- ✅ Rate limiting để chống DDoS
- ✅ Input validation và sanitization
- ✅ CORS configuration
- ✅ HttpOnly cookies

## 🧪 Testing

```bash
# Chạy tests (sẽ được implement)
npm test

# Test coverage
npm run coverage
```

## 📝 License

MIT License - Copyright (c) 2024

## 👥 Contributors

- Your Name - [GitHub](https://github.com/yourusername)

## 📞 Liên Hệ

- Email: info@homerent.vn
- Website: https://homerent.vn
- Facebook: https://facebook.com/homerent

## 🙏 Acknowledgments

- Bootstrap for UI components
- Font Awesome for icons
- OpenAI for AI capabilities
- MySQL & Sequelize for database

## 🔧 Database Schema

### Users Table
```sql
- id (INT, PRIMARY KEY, AUTO_INCREMENT)
- name (VARCHAR)
- email (VARCHAR, UNIQUE)
- phone (VARCHAR)
- password (VARCHAR)
- role (ENUM: user, landlord, admin)
- created_at, updated_at (TIMESTAMP)
```

### Properties Table
```sql
- id (INT, PRIMARY KEY, AUTO_INCREMENT)
- title (VARCHAR)
- description (TEXT)
- price (DECIMAL)
- address_* (VARCHAR)
- landlord_id (INT, FOREIGN KEY -> users.id)
- status (ENUM: available, rented, pending, inactive)
- created_at, updated_at (TIMESTAMP)
```

### Bookings Table
```sql
- id (INT, PRIMARY KEY, AUTO_INCREMENT)
- property_id (INT, FOREIGN KEY -> properties.id)
- tenant_id (INT, FOREIGN KEY -> users.id)
- landlord_id (INT, FOREIGN KEY -> users.id)
- status (ENUM)
- created_at, updated_at (TIMESTAMP)
```

### Reviews Table
```sql
- id (INT, PRIMARY KEY, AUTO_INCREMENT)
- property_id (INT, FOREIGN KEY -> properties.id)
- user_id (INT, FOREIGN KEY -> users.id)
- rating (INT 1-5)
- comment (TEXT)
- created_at, updated_at (TIMESTAMP)
```

---

**Made with ❤️ by HomeRent Team**
